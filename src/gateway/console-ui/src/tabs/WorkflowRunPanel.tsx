    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex gap-2 items-center">
        {!runId ? (
          ...
          <Button size="sm" variant="ghost" onClick={() => { setRunId(null); setRun(null) }}>
            ↩ Run Again
          </Button>
        )}
      </div>
      ...
      {run && (
        <div className="flex flex-col gap-3">
          ...
          {run.stepResults.map((sr, i) => {
            ...
            const stStatus: 'running' | 'success' | 'error' =
              sr.error ? 'error' : sr.output !== undefined ? 'success' : 'running'
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  >
                    {stStatus === 'running' ? '⏳' : stStatus === 'success' ? '✓' : '✗'}{' '}
                    {stStatus}
                  </Badge>
                </div>
                  ...
                  <div className="rounded-lg bg-emerald-950/40 ring-1 ring-emerald-800/40 px-3 py-2 flex flex-col gap-1">
                    ...
                    {Object.entries(newVars).map(([k, v]) => (
                      <div key={k} className="grid grid-cols-[auto_1fr] gap-2 items-start font-mono text-[11px]">
                        ...
                        <span className="text-slate-300 break-all">{v || <span className="text-slate-600 italic">(empty)</span>}</span>
                      </div>
                    ))}
                  </div>
                )}
          ...
          {(() => {
            const latestSnap = [...run.stepResults].reverse().find((s) => s.varsSnapshot && Object.keys(s.varsSnapshot).length > 0)?.varsSnapshot
            ...
            return (
              <div className="rounded-xl bg-slate-800/50 ring-1 ring-slate-700/40 p-4 flex flex-col gap-2">
                <span className="text-xs font-medium text-slate-400 uppercase tracking-wider">📦 所有变量 ({Object.keys(latestSnap).length})</span>
                <div className="grid grid-cols-1 gap-1 font-mono">
                  {Object.entries(latestSnap).map(([k, v]) => (
                    <div key={k} className="grid grid-cols-[minmax(0,auto)_1fr] gap-3 items-start text-[11px]">
