...
export function LogsTab() {
  ...
  const filtered = entries.filter((e) => {
    ...
    const searchOk = !search || e.msg.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] gap-0">
      ...
      <div className="flex-1 overflow-y-auto bg-slate-900/60 rounded-xl ring-1 ring-slate-700/50 p-3 font-mono text-xs min-h-0">
        {filtered.length === 0 && (
          <p className="text-slate-500 text-center py-4">No log entries</p>
        )}
        {filtered.map((entry, i) => (
          <div key={i} className={`flex gap-3 py-0.5 hover:bg-slate-800/40 rounded ${LEVEL_TEXT[entry.level ?? 'info']}`}>
            <span className="text-slate-600 shrink-0 tabular-nums">
              {new Date(entry.ts).toLocaleTimeString()}
            </span>
            <span className="shrink-0 w-12">
              <Badge variant={LEVEL_VARIANT[entry.level ?? 'info'] as 'green'}>{entry.level ?? 'info'}</Badge>
            </span>
