...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start gap-4 py-2.5" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
...
export function AboutTab() {
  const { data } = useQuery<GatewayStatus>(
    () => rpc<GatewayStatus>('gateway.status'),
    [],
  )
    ...
    <div className="flex flex-col gap-6 max-w-2xl">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[13px] text-slate-500">Decentralized, cross-platform, multi-host federated AI Agent framework</p>
        </div>
      </div>
      ...
      >
        <div className="pt-2.5 pb-1">
          <h2 className="text-[11px] font-medium text-slate-500 uppercase tracking-wider">Runtime</h2>
        </div>
        {data ? (
          <>
      ...
      <div
        ...
        style={{ border: '1px solid rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.02)' }}
      >
        <h2 className="text-[11px] font-medium text-slate-500 uppercase tracking-wider mb-2">License</h2>
