...
import { openDatabase, type AnyDatabase } from '../core/runtime-compat.js';
...
export class MemoryStore {
  ...
  deleteByKey(key: string, partition = 'shared'): boolean {
    const result = this.conn.prepare(
      `DELETE FROM memories WHERE key = ? AND partition = ?`,
    ).run(key, partition);
  ...
  loadEmbedding(memoryId: MemoryEntryId): Float32Array | null {
    const row = this.conn.prepare(`SELECT embedding FROM memory_embeddings WHERE memory_id = ?`).get(memoryId) as { embedding: Buffer } | undefined;
    if (!row) return null;
  ...
  listEmbeddedIds(): MemoryEntryId[] {
    const rows = this.conn.prepare(`SELECT memory_id FROM memory_embeddings`).all() as Array<{ memory_id: string }>;
